(() => {
  ...
  function render() {
    ...
    const map = {home:renderHome,today:renderToday,tasks:renderTasks,calendar:renderCalendar,finance:renderFinance,habits:renderHabits,kanban:renderKanban,assistant:renderAssistant,insights:renderInsights,settings:renderSettings};
  ...
  function saveTaskForm(e) {
    ...
    const data = {title:$('task-title').value.trim(),note:$('task-note').value.trim(),priority:$('task-priority').value,project:$('task-project').value,due:$('task-due').value,estimate:Number($('task-estimate').value)||0,reminder:$('task-reminder').value,repeat:$('task-repeat').value,private:$('task-private').checked,subtasks};
  ...
  function openHabitForm(id=null) {
    ...
    $('habit-id').value=h?.id||''; $('habit-name').value=h?.name||''; $('habit-icon').value=h?.icon||'✨'; $('habit-goal').value=h?.goal||1; $('habit-unit').value=h?.unit||'vez'; $('habit-color').value=h?.color||'#00d68f'; $('delete-habit').style.visibility=h?'visible':'hidden'; openModal('habit-modal');
  }
  function saveHabitForm(e) { e.preventDefault(); const id=$('habit-id').value; const old=state.habits.find(x=>x.id===id); const data={name:$('habit-name').value.trim(),icon:$('habit-icon').value.trim()||'✨',goal:Number($('habit-goal').value)||1,unit:$('habit-unit').value.trim()||'vez',color:$('habit-color').value}; if(!data.name)return; if(old)Object.assign(old,data); else state.habits.push({id:uid('habit'),log:{},...data}); closeModals(); sync(); toast('Hábito guardado'); }
  ...
  function deleteItem(type, id) {
    const map = {task:'tasks',event:'events',finance:'transactions',habit:'habits'}; const arr = state[map[type]]; const i = arr.findIndex(x=>x.id===id); if(i<0)return;
  ...
  function undoDelete() {
    ...
    const map={task:'tasks',event:'events',finance:'transactions',habit:'habits'}; state[map[state.undo.type]].push(state.undo.item); state.undo=null; sync(); toast('Deshecho');
  }
  ...
  function assistantReply(input, apply=true) {
    ...
    if (/^(crea|crear|nueva|agrega|agregar) tarea/.test(low)) {
      ...
      if (apply) state.tasks.push({id:uid('task'),title,note:'Creada desde asistente',priority:low.includes('urgente')?'alta':'media',project:'personal',due:date,estimate:extractMinutes(low)||25,reminder:'',repeat:'never',status:'todo',private:false,done:false,createdAt:today(),completedAt:null,xpAwarded:false,subtasks:[]});
    ...
    if (/^(crea|crear|nuevo|agrega|agregar) evento/.test(low)) {
      const time = (low.match(/\b([01]?\d|2[0-3]):([0-5]\d)\b/)||[])[0] || '09:00';
      ...
      if (apply) state.events.push({id:uid('event'),title,location:'',date,start:time,end:addMinutes(time,60),color:state.settings.accent});
  ...
  function bindGlobalEvents() {
    ...
    $('task-form').addEventListener('submit', saveTaskForm); $('event-form').addEventListener('submit', saveEventForm); $('finance-form').addEventListener('submit', saveFinanceForm); $('habit-form').addEventListener('submit', saveHabitForm);
    $('delete-task').addEventListener('click',()=>deleteItem('task',$('task-id').value)); $('delete-event').addEventListener('click',()=>deleteItem('event',$('event-id').value)); $('delete-finance').addEventListener('click',()=>deleteItem('finance',$('finance-id').value)); $('delete-habit').addEventListener('click',()=>deleteItem('habit',$('habit-id').value));
